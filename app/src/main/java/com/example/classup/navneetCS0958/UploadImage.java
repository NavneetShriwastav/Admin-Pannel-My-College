...
public class UploadImage extends AppCompatActivity {
    ...
    private void uploadImage() {
        ...
        // Generating a unique file name for the image
        String imageName = UUID.randomUUID().toString() + ".jpg";

        // Set the file path to the appropriate folder based on the selected category
        switch (category) {
            case "Convocation":
                filePath = storageReference.child("Convocation").child(imageName);
                break;
            case "Independence Day":
                filePath = storageReference.child("Independence Day").child(imageName);
                break;
            case "Other Events":
                filePath = storageReference.child("Other Events").child(imageName);
                break;
            default:
                // Default case
                filePath = null;
        ...
        // Check if filePath is null before proceeding
        if (filePath != null) {
            ...
            Toast.makeText(UploadImage.this, "Invalid category", Toast.LENGTH_SHORT).show();
        }
    }

    private void uploadData(String downloadUrl) {
        // Create a reference to the child node directly
        DatabaseReference categoryReference = reference.child(category);
        final String uniqueKey = categoryReference.push().getKey();
